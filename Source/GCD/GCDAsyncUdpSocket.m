...
NSString *const GCDAsyncResolveHostQueue = @"GCDAsyncUdpSocket";
...
@interface GCDAsyncUdpSocket ()
{
    ...
    dispatch_queue_t resolveHostQueue;
...
- (id)initWithDelegate:(id <GCDAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
    ...
    if ((self = [super init]))
    {
        ...
        else
        {
            ...
            resolveHostQueue = dispatch_queue_create([GCDAsyncResolveHostQueue UTF8String], NULL);
        }
...
#if !OS_OBJECT_USE_OBJC
    if (resolveHostQueue) dispatch_release(resolveHostQueue);
#endif
    resolveHostQueue = NULL;
     ...
     withCompletionBlock:(void (^)(NSArray *addresses, NSError *error))completionBlock
{
    ...
    dispatch_async(resolveHostQueue, ^{ @autoreleasepool {
